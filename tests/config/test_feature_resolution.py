...
def test_for_model_applies_full_override() -> None:
    ...
    fc = FeatureConfig(
        ...
        windows=[10],
        lag_steps=[1],
        ridge=ModelFeatureConfig(feature_families=["raw", "rolling_mean"], windows=[2]),
        gru=ModelFeatureConfig(feature_families=["rolling_mean"], windows=[15]),
    )

    ridge = fc.for_model("ridge")
    assert ridge.feature_families == ["raw", "rolling_mean"]
...
def test_for_model_applies_feature_family_override() -> None:
    """A model override can replace shared feature families."""
    fc = FeatureConfig(
        feature_families=["raw", "lag"],
        windows=[5],
        lag_steps=[1],
        gru=ModelFeatureConfig(
    ...
    ridge = fc.for_model("ridge")
    assert ridge.feature_families == ["raw", "lag"]
    assert ridge.windows == [5]

    gru = fc.for_model("gru")
    assert gru.feature_families == ["raw", "rolling_mean"]
    assert gru.windows == [10]
    assert gru.lag_steps == [1]
...
def test_for_model_override_does_not_leak_across_models() -> None:
    ...
    fc = FeatureConfig(
        feature_families=["rolling_mean"],
        windows=[10],
        ridge=ModelFeatureConfig(feature_families=["raw", "rolling_mean"], windows=[8]),
    )

    assert fc.for_model("gru").feature_families == ["rolling_mean"]
...
def test_model_feature_config_accepts_rolling_std() -> None:
    """rolling_std is a valid FeatureFamilyName for ModelFeatureConfig."""
    cfg = ModelFeatureConfig(feature_families=["rolling_std"])  # type: ignore[arg-type]
    assert cfg.feature_families == ["rolling_std"]


def test_model_feature_config_accepts_rolling_slope() -> None:
    """rolling_slope is a valid FeatureFamilyName for ModelFeatureConfig."""
    cfg = ModelFeatureConfig(feature_families=["rolling_slope"])  # type: ignore[arg-type]
    assert cfg.feature_families == ["rolling_slope"]


def test_model_feature_config_accepts_rolling_delta() -> None:
    """rolling_delta is a valid FeatureFamilyName for ModelFeatureConfig."""
    cfg = ModelFeatureConfig(feature_families=["rolling_delta"])  # type: ignore[arg-type]
    assert cfg.feature_families == ["rolling_delta"]


def test_model_feature_config_accepts_rolling_min() -> None:
    """rolling_min is a valid FeatureFamilyName for ModelFeatureConfig."""
    cfg = ModelFeatureConfig(feature_families=["rolling_min"])  # type: ignore[arg-type]
    assert cfg.feature_families == ["rolling_min"]


def test_model_feature_config_accepts_rolling_max() -> None:
    """rolling_max is a valid FeatureFamilyName for ModelFeatureConfig."""
    cfg = ModelFeatureConfig(feature_families=["rolling_max"])  # type: ignore[arg-type]
    assert cfg.feature_families == ["rolling_max"]
...
def test_feature_config_for_model_rolling_std_gru() -> None:
    ...
    fc = FeatureConfig(
        gru=ModelFeatureConfig(feature_families=["rolling_std"]),  # type: ignore[arg-type]
    )
    resolved = fc.for_model("gru")
    assert resolved.feature_families == ["rolling_std"]
...
def test_feature_config_for_model_rolling_slope_ridge() -> None:
    ...
    fc = FeatureConfig(
        ridge=ModelFeatureConfig(feature_families=["rolling_slope"]),  # type: ignore[arg-type]
    )
    resolved = fc.for_model("ridge")
    assert resolved.feature_families == ["rolling_slope"]
...
def test_feature_config_for_model_rolling_delta_lstm() -> None:
    ...
    fc = FeatureConfig(
        lstm=ModelFeatureConfig(feature_families=["rolling_delta"]),  # type: ignore[arg-type]
    )
    resolved = fc.for_model("lstm")
    assert resolved.feature_families == ["rolling_delta"]
...
def test_feature_config_for_model_rolling_min_gru() -> None:
    ...
    fc = FeatureConfig(
        gru=ModelFeatureConfig(feature_families=["rolling_min"]),  # type: ignore[arg-type]
    )
    resolved = fc.for_model("gru")
    assert resolved.feature_families == ["rolling_min"]
...
def test_feature_config_for_model_rolling_max_ridge() -> None:
    ...
    fc = FeatureConfig(
        ridge=ModelFeatureConfig(feature_families=["rolling_max"]),  # type: ignore[arg-type]
    )
    resolved = fc.for_model("ridge")
    assert resolved.feature_families == ["rolling_max"]
